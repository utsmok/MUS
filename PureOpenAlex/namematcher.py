...
class NameMatcher:
    ...
    def __init__(self, distfun="levenshtein"):
        if distfun == "levenshtein":
            ...
            self.distfun = lambda str1, str2: 1 - jaro_winkler_similarity(str1, str2)
        else:
        ...
        self.params = {
            "weights": [0.1, 0.8, 0.1],
            "disc_abbrev": 0.95,
            "disc_abbrev_notstart": 0.4,
            "disc_missing_fname": 0.3,
            "disc_missing_mname": 0.98,
            "disc_missing_nickname": 0.3,
            "disc_initial": 0.9,
            "disc_initial_mismatch": 0.2,
            "disc_acronym": 0.8,
        }
    ...
    def _levenshtein_log(self, str1, str2):
        ...
        log_edits = 1 - log(
            max(len(str1), len(str2)) - edits + 1, max(len(str1), len(str2)) + 1
        )
        return log_edits

    def match_names(self, name1, name2, speed="slow", min_last_sim=0.8):
        ...
        sims["last_name"] = 1 - self.distfun(
            name_dict1["last_name"], name_dict2["last_name"]
        )
        if speed == "fast" and sims["last_name"] < min_last_sim:
            sims["first_names"] = 1 - self.distfun(
                " ".join(name_dict1["last_name"]), " ".join(name_dict2["last_name"])
            )
        ...
        if (name_dict1["nicknames"] and not name_dict2["nicknames"]) or (
            name_dict2["nicknames"] and not name_dict1["nicknames"]
        ):
    ...
    def parse_name(self, name):
        ...
        for part in parts:
            ...
            if nickname_match:
                ...
                name_dict["first_names"].append(part)
        return name_dict
    ...
    def _suffix_sim(self, suffix1, suffix2):
        ...
        if s1 == s2:
            return 1
        if any(s == "junior" for s in [s1, s2]) and any(s == "jr" for s in [s1, s2]):
            return 1
        if any(s == "senior" for s in [s1, s2]) and any(s == "sr" for s in [s1, s2]):
            return 1
        if not s1 or not s2:
            return 0.5
        return 0
                ...
                if (
                    ...
                    if shorter[t][0] == longer[s + t][0]:
                        ...
                        if last_initial_match:
                            if (len(shorter[t]) == 1 and shorter[t][0] == jrlong) or (
                                len(longer[s + t]) == 1 and longer[s + t][0] == jrshort
                            ):
                                token_sims.append(self.params["disc_acronym"])
                        else:
                            token_sims.append(self.params["disc_initial_mismatch"])
                        last_initial_match = False
                else:
                    ...
                    if (
                        ...
                        and (
                            (
                                s + t + 1 < len(longer)
                                and shorter[t][1] == longer[s + t + 1][0]
                            )
                        ...
                        len(longer[s + t]) == 2
                        and longer[s + t][0] == shorter[t][0]
                        and (
                            (
                                t + 1 < len(shorter)
                                and longer[s + t][1] == shorter[t + 1][0]
                            )
                    ...
                    last_initial_match = False
            if token_sims:
                sequence_sims.append(mean(token_sims))
        sim = max(sequence_sims)
        if sequence_sims[0] != sim:
            sim *= self.params["disc_missing_fname"]
            ...
            if (
                ...
                sim *= self.params["disc_missing_mname"]
        return sim
    ...
    def find_closest_names(self, target_names, other_names):
        ...
        others_enum = [
            (self._name_string_std(name_dict), name_dict, i)
            for i, name_dict in enumerate(others_dicts)
        ]
        others_sorted = sorted(others_enum)
        """		print("\n====================\n")
		print("input details for namematching:")
		print("\nTarget input #:",len(target_names))
		print("\nPop input #:",len(other_names))
		print("\nTarget dict #:",len(targets_dicts))
		print("\nPopulation dict #:",len(others_dicts))
		print("\nSorted population dict #:",len(others_sorted))
		print("\nFirst 5 target inputs:",target_names[0:4])
		print("\nFirst 5 pop inputs:",other_names[0:4])
		print("\nFirst 5 target dict entries:",targets_dicts[0:4])
		print("\nFirst 5 pop dict entries:",others_dicts[0:4])
		print("\nFirst 5 sorted pop dict entries:",others_sorted[0:4])
		print("\n Type of other_names:",type(other_names))
		print("\nOther names 260 to 270:",other_names[260:270])
		print("\n====================\n")"""
        matches = []
        for target_dict in targets_dicts:
            ...
            while cont_left or cont_right:
                if cont_left and i > 0:
                    i -= 1
                    sims = {
                        "suffix": self._suffix_sim(
                            target_dict["suffix"], others_sorted[i][1]["suffix"]
                        ),
                        "last_name": 1
                        - self.distfun(
                            target_dict["last_name"], others_sorted[i][1]["last_name"]
                        ),
                        "first_names": 1,
                    }
                    if self._weighted_sum(sims) < best_sim:
                        ...
                        sim = self.match_names(
                            target_std, others_sorted[j][0], speed="slow"
                        )
                        if sim > best_sim:
                ...
                if cont_right and j < len(others_sorted) - 1:
                    ...
                    sims = {
                        "suffix": self._suffix_sim(
                            target_dict["suffix"], others_sorted[j][1]["suffix"]
                        ),
                        "last_name": 1
                        - self.distfun(
                            target_dict["last_name"], others_sorted[j][1]["last_name"]
                        ),
                        "first_names": 1,
                    }
                    if self._weighted_sum(sims) < best_sim:
