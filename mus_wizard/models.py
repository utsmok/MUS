...
class Tag(TimeStampedModel, models.Model):
    class TagTypes(models.TextChoices):
        ...
        INSTITUTE_AUTHOR_MATCH = 'IA'  # at least one of the authors of work has affiliation w/ primary institution
        # in the publication year of the work
...
class OrganizationTopic(MusModel):
    organization = models.ForeignKey('Organization', on_delete=models.CASCADE, related_name="organization_topics",
                                     db_index=True)
...
# TODO: Organization types idea: use tags of tagtype 'ORG_TYPE' ('OT') to store the type of this institution instance
#  + all related institutions (see 'roles' field in api data) note: double check for accidental duplicates when going
#  through the list of related items!! the actual type should be in the 'notes' field of the tag these are the types
#  that can be used: education, funder, healthcare, company, archive, nonprofit, government, facility, other
...
class Author(MusModel):
    ...
    alternative_names = models.JSONField(encoder=DjangoJSONEncoder,
                                         null=True)  # e.g. display_name_alternatives from openalex or
    # default_publishing_name from pure
...
class Affiliation(MusModel):
    ...
    organization = models.ForeignKey(Organization, on_delete=models.CASCADE, related_name="affiliation_details",
                                     db_index=True)
