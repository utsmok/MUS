...
class OAI_PMH(GenericAPI):
    ...
    async def get_work_references(self, cerif:dict[list]|dict[dict]) -> tuple[str, list[dict]|None]:
        async def map_values(value:dict) -> dict:
            ...
            result = {
                'internal_repository_id':value.get('@id'),
                'peer_reviewed':value.get('pubt:Type').get('@pure:peerReviewed') if value.get('pubt:Type') else None,
                'publication_category':value.get('pubt:Type').get('@pure:publicationCategory') if value.get('pubt:Type') else None,
    ...
    async def get_work_publishers(self, cerif:dict[list]|dict[dict]) -> tuple[str, list[dict]|None]:
        ...
        try:
            if isinstance(cerif, dict):
                ...
                return fieldname, [c.get('cerif:Publisher').get('cerif:OrgUnit').get('cerif:Name').get('#text') for c in cerif]
        except Exception as e:
        ...
        async def map_values(value:dict) -> dict:
            ...
            if affiliations:
                ...
                for affil in affiliations:
                    ...
                    affils.append({
                        'internal_repository_id':affil.get('cerif:OrgUnit').get('@id') if affil.get('cerif:OrgUnit') else None,
                        'name':affil.get('cerif:OrgUnit').get('cerif:Name').get('#text') if affil.get('cerif:OrgUnit') else None,
                        'acronym':affil.get('cerif:OrgUnit').get('cerif:Acronym') if affil.get('cerif:OrgUnit') else None,
                    })
                result['affiliations'] = affils
            person = value.get('cerif:Person')
            if person:
                result['internal_repository_id'] = person.get('@id')
                result['family_names'] = person.get('cerif:PersonName').get('cerif:FamilyNames') if person.get('cerif:PersonName') else None
                result['first_names'] = person.get('cerif:PersonName').get('cerif:FirstNames') if person.get('cerif:PersonName') else None
            return result
        fieldname = 'authors'
            ...
            if affiliations:
                ...
                for affil in affiliations:
                    affils.append({
                        ...
                        'name':affil.get('cerif:Name').get('#text') if affil.get('cerif:Name') else None,
        ...
        if not isinstance(cerif, dict) and not isinstance(cerif, list):
            ...
            for i in cerif:
                ...
                for date, vals in i.items():
                    res[str(date).split(':')[1].lower()] = {'start':vals.get('@startDate'), 'end':vals.get('@endDate')}
                result.append(res)
    ...
    async def get_dataset_generated_by(self, cerif:dict[list]|dict[dict]) -> tuple[str, list[dict]|None]:
        ...
        if not isinstance(cerif, dict) and not isinstance(cerif, list):
            ...
            result = [{
                'internal_repository_id':cerif.get('cerif:Equipment').get('@id') if cerif.get('cerif:Equipment') else None,
                'name':cerif.get('cerif:Equipment').get('cerif:Name').get('#text') if cerif.get('cerif:Equipment') else None,
            }]
        elif isinstance(cerif, list):
            result = []
            for i in cerif:
                result.append({
                    ...
                    'name':i.get('cerif:Equipment').get('cerif:Name').get('#text') if i.get('cerif:Equipment') else None,
                })
        return fieldname, result
    async def get_dataset_publishers(self, cerif:dict[list]|dict[dict]) -> tuple[str, list[dict]|None]:
        ...
        if not isinstance(cerif, dict) and not isinstance(cerif, list):
            ...
            result = [{
                ...
                'org_unit_name':cerif.get('cerif:OrgUnit').get('cerif:Name').get('#text') if cerif.get('cerif:OrgUnit') else None,
            }]
        elif isinstance(cerif, list):
            result = []
            for i in cerif:
                ...
                'org_unit_name':i.get('cerif:OrgUnit').get('cerif:Name').get('#text') if i.get('cerif:OrgUnit') else None,
                })
        return fieldname, result
    async def get_dataset_originates_from(self, cerif:dict[list]|dict[dict]) -> tuple[str, list[dict]|None]:
        async def map_values(value:dict) -> dict:
            ...
            if funderdict:
                funder = {
                    'internal_repository_id':funderdict.get('cerif:OrgUnit').get('@id') if funderdict.get('cerif:OrgUnit') else None,
                    'acronym':funderdict.get('cerif:OrgUnit').get('cerif:Acronym') if funderdict.get('cerif:OrgUnit') else None,
                    'name':funderdict.get('cerif:OrgUnit').get('cerif:Name').get('#text') if funderdict.get('cerif:OrgUnit') else None,
                }
            return {
                'internal_repository_id':value.get('cerif:Equipment').get('@id') if value.get('cerif:Equipment') else None,
                'funder':funder,
                'identifier':value.get('cerif:Identifier').get('#text') if value.get('cerif:Identifier') else None,
                'identifier_type':value.get('cerif:Identifier').get('@type').split('/dk/atira/funding/fundingdetails/')[-1] if value.get('cerif:Identifier') else None,
    ...
    async def process_cerif(self, type:str, data:list[dict]) -> list[dict]:
        keys_missing = set()
        results=[]
    ...
    async def get_results(self, type:str, url:str, collection:motor.motor_asyncio.AsyncIOMotorCollection) -> tuple[list[dict], list[str]]:
        results = {
            'raw':[],
            'processed':[],
            'missing_keys':[],
        ...
        while True:
            ...
            try:
                for subitem in tmp[0]:
                    await collection.find_one_and_update({"internal_repository_id":subitem['internal_repository_id']}, {'$set':subitem}, upsert=True)
            except Exception as e:
    ...
    async def get_item_results(self):
        ...
        all_itemsets = {'works':'openaire_cris_publications','persons':'openaire_cris_persons', 'orgs':'openaire_cris_orgunits','datasets':'datasets:all'}
        itemsets = [(k,v) for k,v in all_itemsets.items()]
        
        async with aiometer.amap(functools.partial(self.call_api), itemsets, max_at_once=self.api_settings['max_at_once'], max_per_second=self.api_settings['max_per_second']) as responses:
    ...
    CERIF_RESULT_KEYLIST = {
        ...
        'works': ['cerif:Keyword', 'cerif:Editors', 'cerif:Subtitle', 'cerif:Publishers', 'cerif:Authors', 'cerif:DOI', 'cerif:ISBN','cerif:URL', 'cerif:SCP-Number', 'cerif:Title', 'cerif:Status', 'cerif:FileLocations', 'cerif:PresentedAt', 'cerif:References', 'cerif:Abstract', 'cerif:Language', 'cerif:PublicationDate'],
        'datasets': ['cerif:Creators', 'cerif:Name', 'cerif:GeneratedBy', 'cerif:FileLocations', 'cerif:Dates', 'cerif:OriginatesFrom','cerif:DOI', 'cerif:URL', 'cerif:References', 'cerif:Publishers', 'cerif:License', 'cerif:Description'],
        'products': [],
        'patents': [],
        'projects': [],
        'funding': [],
